...
// --- Componente do Card de Problema ---
const ProblemaCard = ({ item, colors, onEdit, onDelete }) => {
    ...
    const translateTipoProblema = (tipo) => {
        switch (tipo) {
            case 'MECANICO': return t('problemTypeMechanical');
            case 'ELETRICO': return t('problemTypeElectrical');
            case 'CARROCERIA': return t('problemTypeBodywork');
            case 'SEGURANCA': return t('problemTypeSecurity');
            case 'LEGAL': return t('problemTypeLegal');
            case 'OUTRO': return t('problemTypeOther');
            default: return tipo;
        }
    };

    const translateStatus = (resolvido) => {
        return resolvido ? t('problemStatusResolved', 'Resolvido') : t('problemStatusPending', 'Pendente');
    };

    return (
        <View style={styles.problemaCard}>
            <View style={{flex: 1}}>
                <Text style={styles.problemaTipo}>{translateTipoProblema(item.tipoProblema)}</Text>
                <Text style={styles.problemaDescricao}>{item.descricao}</Text>
                <Text style={styles.problemaData}>{t('labelReportedOn', 'Reportado em:')} {item.dataRegistro}</Text>
                <Text style={styles.problemaStatus(item.resolvido)}>{translateStatus(item.resolvido)}</Text>
            </View>
            <View style={styles.problemaActions}>
                <TouchableOpacity onPress={onEdit} style={styles.actionButton}>
                    <Text style={[styles.actionButtonText, { color: colors.primary }]}>{t('editButton')}</Text>
                </TouchableOpacity>
                <TouchableOpacity onPress={onDelete} style={styles.actionButton}>
                    <Text style={[styles.actionButtonText, { color: colors.danger }]}>{t('deleteButton')}</Text>
                </TouchableOpacity>
            </View>
        </View>
    );
...
// --- Tela principal da Lista de Problemas ---
export default function ProblemasListScreen({ route, navigation }) {
    ...
    React.useLayoutEffect(() => {
        navigation.setOptions({ title: `${t('labelProblemasList')} - ${motoPlaca}` });
        ...
        <SafeAreaView style={styles.safeArea}>
            <View style={styles.header}>
                <Button 
                    ...
                    onPress={() => navigation.navigate('RegistrarProblema', { motoId: motoId })} 
                ...
                <FlatList
                    ...
                    renderItem={({ item }) => (
                        <ProblemaCard
                            ...
                            onEdit={() => navigation.navigate('EditarProblema', { problema: item, motoId: motoId })}
                        />
                    )}
                    ListEmptyComponent={
